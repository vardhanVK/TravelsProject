...
public class Bill {
	...
	public static void printBill(Bus bus, List<Object[]> availableBuses, String boardingStation, String destination,
			List<Passenger> passengersList, LocalDate date) {
		printHorizontal();
		System.out.printf("%n|%38s%36s%n", "Bill", "|");
		printHorizontal();
		System.out.printf("%n| %-10s%-20S-%20S%14S%6S%2s", bus.getBusNumber(), boardingStation, destination,
				bus.getCategory(), bus.getBusType(), "|");
		printPassengers(passengersList);
		int rem = 13 - (passengersList.size() * 2);
		printVertical(rem);
		double price = PlanJourney.getPrice(bus, getDistance(availableBuses, bus.getBusNumber()));
	...
	private static void printPrice(double price, LocalDate date) {
		...
		System.out.printf(Constants.FAREFORMAT, "|", "Base Fare", price, "|");
		System.out.printf(Constants.FAREFORMAT, "|", "weekend surge", weekendSurge, "|");
		System.out.printf(Constants.FAREFORMAT, "|", "gst", gst, "|");
		System.out.printf("%n%-65s--------%2s", "|", "|");
		System.out.printf(Constants.FAREFORMAT, "|", "Total", total, "|");
		System.out.printf("%n%-65s--------%2s%n", "|", "|");
	}

	private static int getDistance(List<Object[]> availableBuses, int busNumber) {
		for (Object[] bus : availableBuses) {
			if (((int) bus[0]) == busNumber)
				return ((int) bus[4]) - ((int) bus[2]);
		}
		return 0;
	}
}
